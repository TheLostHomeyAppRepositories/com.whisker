...
class DataManager {
   ...
   * @private
   */
  _notifyWeightUpdate(weight, sourceDeviceId) {
    if (this.weightUpdateSubscribers.size === 0) {
      return;
    }

    const weightGrams = Math.round(weight * 453.59237);
    this.homey.log(`Weight update: ${weight} lbs (${weightGrams} g) from device ${sourceDeviceId}`);
    ...
    for (const callback of this.weightUpdateSubscribers) {
      try {
        ...
        this.homey.error('Error in weight update callback:', error);
      }
    }
  }
  ...
  _handleWeightUpdate(deviceId, weightData) {
    this.homey.log(`Pet device ${deviceId} received weight update:`, weightData);
