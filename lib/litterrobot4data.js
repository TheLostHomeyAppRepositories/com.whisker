...
/**
 * Comprehensive data wrapper for Litter-Robot 4 devices that handles all data interpretation,
 * formatting, and status mapping without making API calls.
 */
...
class LitterRobot4Data {
  ...
  /** Days of week array for sleep schedule calculations */
  ...
  /** Maps raw robot status codes to simplified LitterBox status codes */
  ...
  /** Maps derived status codes to human-readable descriptions for user interface display */
  ...
  /** Maps problem codes to user-friendly descriptions for error reporting and troubleshooting */
  ...
  /** Maps internal command names to API endpoint commands for consistent robot control */
  ...
  /** Centralized error messages for consistent error handling and user feedback */
  ...
  /**
   * Creates a new LitterRobot4Data instance for processing robot state data.
   * Validates input parameters and initializes caching for performance optimization.
   * @param {Object} params
  ...
  constructor({ robot, settings = {} } = {}) {
    // Validate robot data to ensure proper data structure for processing
    ...
    // Convert time format settings to boolean for consistent processing across different input formats
    ...
    // Initialize caching system for performance optimization of frequently accessed computed values
  ...
  /**
   * Checks if robot data has changed and invalidates cache when necessary.
   * Ensures cached values remain accurate while maintaining performance benefits.
   * @private
   */
  ...
  /**
   * Formats Date objects with consistent options for display across the application.
   * Provides standardized date/time formatting to ensure UI consistency.
   ...
   * This is the main method for all time/date formatting in the app, supporting
   * timezone conversion and user preference settings.
  ...
  static formatTime(timeInput, { 
    ...
    if (typeof timeInput === 'string') {
      // Handle ISO string timestamps (like lastSeen) which are in UTC
      ...
      // Handle minutes from midnight (like sleep schedule times)
  ...
  /**
   * Formats firmware version information to match the Whisker app display format.
   * Combines multiple firmware components into a single, user-friendly version string.
  ...
  static formatFirmwareVersion(robot) {
    try {
      ...
      // Extract ESP firmware version by removing dots: "1.1.75" -> "1175"
      const espVersion = espFirmware.replace(/\./g, '');
      
      // Extract PIC firmware version by combining last two parts: "10500.3072.2.92" -> "292"
      ...
      // Extract laser board firmware by removing dots: "5.0.2.1" -> "5021"
      const laserVersion = laserBoardFirmwareVersion.replace(/\./g, '');
      
      // Combine components in Whisker app format: "1175.5021.292"
  ...
  /**
   * Derives the official Whisker status code from raw robot payload using priority-based logic.
   * Matches the native Whisker app behavior to ensure consistent status reporting across platforms.
  ...
  static deriveStatusCode(robot) {
    ...
    // Destructure robot properties once for performance optimization
    const {
  ...
  /**
   * Derives a user-friendly clean cycle status string for display purposes.
   * Converts technical robot states into understandable descriptions for users.
  ...
  /**
   * Determines if a cat is detected based on robot data by checking multiple indicators.
   * Provides a reliable way to detect cat presence across different robot states.
   * @param {Object} robot - Robot data object
  ...
  /**
   * Analyzes robot status for problems and returns comprehensive problem information.
   * Identifies both robot and hopper-related issues for troubleshooting and user notification.
   * @param {Object} robot - Robot data object
  ...
  static analyzeProblem(robot) {
    ...
    // Check for hopper-related problems that require separate handling
   ...
   * Handles timezone conversion and cross-day sleep schedules for accurate time display.
  ...
  static computeSleepSchedule(robot, { use12hFormat = false } = {}) {
    ...
    try {
      ...
      // Handle cross-day sleep schedules where wake time is before sleep time
      ...
      // Create Date objects for Flow automation triggers with proper timezone handling
  ...
  /**
   * @returns {string} Derived status code with intelligent caching for performance optimization
   */
  get statusCode() {
  ...
  /**
   * @returns {string} User-friendly status description for interface display
   */
  ...
  /**
   * @returns {string} Detailed description for the current robot cycle state for user feedback
   */
  ...
  /**
   * @returns {string} User-friendly night light mode for interface display
   */
  get nightLightMode() {
    ...
    switch (mode) {
      ...
      default:
        // Default to 'off' for unknown or undefined values to ensure consistent behavior
        return 'off';
    }
  }
  ...
  /** @returns {string|null} Panel brightness level (low, medium, high) */
  get panelBrightness() {
    const brightness = this._robot.panelBrightnessHigh;
  ...
  /**
   * @returns {Object|null} Sleep schedule information with intelligent caching for performance
   */
  get sleepSchedule() {
    this._invalidateCacheIfNeeded();
  ...
  /**
   * @returns {string|null} Formatted last seen timestamp with timezone conversion
   */
  ...
  /**
   * @returns {string|null} Formatted setup date/time for user display
   */
  ...
  updateRobotData(newRobotData) {
    // Validate new robot data to ensure proper structure before updating
