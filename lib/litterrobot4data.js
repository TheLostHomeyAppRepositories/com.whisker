...
class LitterRobot4Data {
  ...
  static ProblemDescriptions = Object.freeze({
    ...
    LSD: 'Laser dirty',
    USB: 'USB fault',
    HOPPER_ERROR: 'LitterHopper error',
    HOPPER_MOTOR_FAULT: 'LitterHopper motor fault'
  });
   ...
   */
  _computeRobotHash() {
    const keyProps = ['robotStatus', 'displayCode', 'catDetect', 'isOnline', 'robotCycleStatus'];
   ...
   */
  static formatDateForDisplay(dateObj, { use12hFormat = false, forceUSDate = false } = {}) {
    if (!(dateObj instanceof Date) || isNaN(dateObj)) {
      return 'Invalid date';
    }

    const locale = forceUSDate ? 'en-US' : undefined; // undefined → user locale
  ...
  static formatTime(timeInput, { 
    ...
    return LitterRobot4Data.formatDateForDisplay(dateObj, { use12hFormat, forceUSDate });
  }
  ...
  static deriveStatusCode(robot) {
    ...
    // Priority 7: Hopper faults
    if (hopperStatus === 'MOTOR_FAULT_SHORT' || 
        hopperStatus === 'MOTOR_OT_AMPS' || 
        hopperStatus === 'MOTOR_DISCONNECTED') return 'HOPPER_ERROR';

    // Priority 8: Drawer / bonnet states
    if (isBonnetRemoved) return 'BR';
    if (isDFIFull) return 'DFS';

    // Priority 9: Cycle / cleaning
    ...
    // Priority 10: Idle fallback with displayCode tweaks
    ...
    // Priority 11: Display-code specific
  ...
  static analyzeProblem(robot) {
    ...
    // Check for hopper motor faults only (not empty or disabled)
    const hopperErrors = [];
    if (robot.hopperStatus === 'MOTOR_FAULT_SHORT' || 
        robot.hopperStatus === 'MOTOR_OT_AMPS' || 
        robot.hopperStatus === 'MOTOR_DISCONNECTED') {
      hopperErrors.push('HOPPER_MOTOR_FAULT');
    }
    
    // Combine robot problems with hopper errors
    const allCodes = [...activeCodes, ...hopperErrors];
    
    return {
      hasProblem: allCodes.length > 0,
      codes: allCodes,
      description: allCodes.length ? problemDescriptions[allCodes[0]] : null
    };
  }
   ...
   */
  static computeSleepSchedule(robot, { use12hFormat = false, useUSDate = false } = {}) {
  ...
  /**
   * @returns {string} User-friendly hopper status description
   */
  get hopperStatus() {
    ...
    switch (status) {
      ...
      case 'MOTOR_FAULT_SHORT': return 'motor_fault';
      case 'MOTOR_OT_AMPS': return 'motor_fault';
      case 'MOTOR_DISCONNECTED': return 'motor_fault';
      case 'EMPTY': return 'empty';
      default:
        // Return 'Unknown' for unknown status values
        return 'Unknown';
    }
  }
  ...
  /** @returns {boolean} True if there is a known problem (robot or hopper) */
  get hasProblem() {
  ...
  /** @returns {boolean} True if hopper is removed/disabled */
  get isHopperRemoved() {
    return this._robot.isHopperRemoved === true;
  }
  ...
  get lastSeenFormatted() {
    return LitterRobot4Data.formatTime(this._robot.lastSeen, {
      use12hFormat: this._settings.use12hFormat,
      forceUSDate: this._settings.useUSDate,
