...
/**
 * Data wrapper for Litter-Robot 4 devices that processes robot state data,
...
class LitterRobot4Data {
  ...
  static ErrorMessages = Object.freeze({
    ...
    INVALID_NIGHT_LIGHT_BRIGHTNESS: 'Invalid night light brightness value',
  ...
  /**
   * Formats time values from either ISO strings or minutes-from-midnight format.
  ...
  static formatTime(timeInput, {
    ...
    if (typeof timeInput === 'string') {
      ...
      if (timezone) {
        // Create date in the specified timezone to convert to local time
   ...
   * Combines multiple firmware components (ESP, PIC, laser board) into a single version string.
   ...
   * Matches the native Whisker app behavior for consistent status reporting.
   ...
   * Checks status code, catDetect field, and displayCode to handle different robot states.
   * @param {Object} robot - Robot data object
   ...
   * Identifies both robot and hopper-related issues for troubleshooting and notification.
   * @param {Object} robot - Robot data object
  ...
  static analyzeProblem(robot) {
    ...
    if (hopperStatus) {
      switch (hopperStatus) {
        ...
        default:
          // Unknown hopper status - no action needed
          break;
      }
    }
  ...
  _getProblemAnalysis() {
    ...
    if (this._cachedProblemAnalysis === null) {
      this._cachedProblemAnalysis = LitterRobot4Data.analyzeProblem(this._robot);
    }
  ...
  static computeSleepSchedule(robot, { use12hFormat = false } = {}) {
    ...
    try {
      ...
      // Create Date objects for Flow automation triggers, accounting for timezone
  ...
  /**
   * @returns {string} Night light mode (off, on, or auto) for interface display
   */
  get nightLightMode() {
    ...
    switch (mode) {
      ...
      default:
        // Default to 'off' for unknown or undefined values to maintain consistent behavior
        return 'off';
    }
  }
  ...
  get isOnOff() {
    return this._robot.unitPowerStatus !== 'OFF';
  }
  ...
  /** @returns {string|null} Night light brightness level (low, medium, high) */
  get nightLightBrightnessLevel() {
    const brightness = this._robot.nightLightBrightness;
    if (brightness === 25) return 'low';
    if (brightness === 50) return 'medium';
    if (brightness === 100) return 'high';
    return null;
  ...
  /**
   * @returns {Object|null} Sleep schedule information, cached to avoid recomputation when data hasn't changed
   */
  get sleepSchedule() {
    this._invalidateCacheIfNeeded();
