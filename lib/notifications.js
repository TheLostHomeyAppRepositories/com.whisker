...
const { colorize, LOG_COLORS, convertLbsToGrams, isInvalidCapabilityError } = require('./utils');
...
/**
 * Handles capability errors with consistent messaging.
 * Detects "Invalid Capability" errors and logs appropriate messages.
 * Creates a notification if the logger is a device instance with store methods.
 * @param {Error} err - The error to handle
 * @param {string} capability - The capability name
 * @param {string} action - The action being performed (e.g., "initialize", "update", "add", "remove")
 * @param {Object} logger - The logger object with error method (may be a device instance)
 * @returns {void}
 */
function handleCapabilityError(err, capability, action, logger) {
  if (!err) return;

  if (isInvalidCapabilityError(err)) {
    logger.error(`[Capability] ${colorize(LOG_COLORS.ERROR, `Error trying to ${action} capability `
      + `${LOG_COLORS.BOLD}${capability}${LOG_COLORS.RESET}${LOG_COLORS.ERROR} which is missing for this device, `
      + 'please remove the device and re-pair to fix this issue')}`);

    if (logger && typeof logger.getStoreValue === 'function' && logger.homey) {
      createUpdateNotification(logger, logger.homey).catch((notificationError) => {
        // Don't fail if notification creation fails, just log it
        logger.error(`[Capability] ${colorize(LOG_COLORS.ERROR, 'Failed to create update notification:')}`, notificationError);
      });
    }
  } else {
    logger.error(`[Capability] ${colorize(LOG_COLORS.ERROR, `Failed to ${action} capability ${LOG_COLORS.BOLD}${capability}${LOG_COLORS.RESET}${LOG_COLORS.ERROR}:`)}`, err);
  }
}
