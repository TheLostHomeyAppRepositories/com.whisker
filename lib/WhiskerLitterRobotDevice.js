...
module.exports = class WhiskerLitterRobotDevice extends WhiskerDevice {
  ...
  async onInit() {
    ...
    try {
      ...
      await this.setUnavailable(err instanceof WhiskerNetworkException
        ? this.homey.__('errors.network_unreachable')
        : err.message)
        .catch((setErr) => this.error('Failed to set device unavailable:', setErr));
    }
  }
    ...
    return super.registerCapabilityListener(capability, async (...args) => {
      try {
        ...
        if (err instanceof WhiskerNetworkException) {
          await this.setUnavailable(this.homey.__('errors.network_unreachable'))
            .catch((setErr) => this.error('Failed to set device unavailable:', setErr));
        }
        throw err;
      }
  ...
  async _setupWebSocket() {
    ...
    await this.setAvailable()
      .catch((err) => this.error('Failed to set device available:', err));
