...
class ApiSession extends EventEmitter {
  ...
  async createWebSocketConnection(deviceId, options = {}) {
    if (this.websocketConnections.has(deviceId)) {
      this.log(`[ApiSession] ${colorize(LOG_COLORS.WARNING, `WebSocket connection already exists for device ${deviceId}`)}`);
    ...
    return new Promise((resolve, reject) => {
      ...
      ws.on('error', (error) => {
        this.homey.error(`[ApiSession] ${colorize(LOG_COLORS.ERROR, `WebSocket error for device ${deviceId}:`)}`, error);
        this.eventEmitter.emitError(error, `websocket_error_${deviceId}`);
        reject(error);
      });
      
      ws.on('close', (code, reason) => {
        this.homey.log(`[ApiSession] ${colorize(LOG_COLORS.WARNING, `WebSocket closed for device ${deviceId}: ${code} ${reason}`)}`);
